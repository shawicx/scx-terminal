...
function rgbToLab (rgb: RGB): LAB {
    ...
    const y2 = y > 0.008856 ? Math.cbrt(y) : 7.787 * y + 16.0 / 116.0
    z = z > 0.008856 ? Math.cbrt(z) : 7.787 * z + 16.0 / 116.0

    return { l: 116.0 * y2 - 16.0, a: 500.0 * (x - y2), b: 200.0 * (y2 - z) }
}
...
function labToRgb (lab: LAB): RGB {
    ...
    let g = -xf * 0.969266 + yf * 1.8760108 + zf * 0.0415560
 ...
 * using LAB color space interpolation. Ported from tabby-terminal.
 */
