...
//! unacknowledged bytes exceed the queue's window, the reader thread stops
//! draining the pty, which makes the kernel block the child process (real
//! backpressure). 队列与 UTF-8 切分实现在 queue 子模块。
...
pub(crate) use queue::PtyDataQueue;
